...
The examples above use `run_program` with CQL source strings. CQL.jl also provides Julia-native macros. Here is how the PropertyGraph and TripleStore schemas look using the DSL:
...
Note: The `schema_colimit` (unified view), mappings with `lambda` expressions (`ToTriples`, `ToReified`), and sigma/delta migrations still require the `cql"""..."""` or `run_program` syntax. The DSL covers individual schema and instance definitions.
