...
The examples above use `run_program` with CQL source strings. CQL.jl also provides Julia-native macros. Here is how the PropertyGraph and TripleStore schemas look using the DSL, along with a schema colimit to merge them:
...
The `@schema_colimit` macro can merge the two paradigms by identifying corresponding entities:
...
Note: Mappings with `lambda` expressions (`ToTriples`, `ToReified`) and observation equations in colimits still require the `cql"""..."""` or `run_program` syntax. The DSL covers schema and instance definitions, plus entity-level schema colimits.
