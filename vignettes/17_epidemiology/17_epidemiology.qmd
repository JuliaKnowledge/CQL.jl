...
The examples above use `run_program` with CQL source strings. CQL.jl also provides Julia-native macros. Here is how the Epi schema looks using the DSL:
...
Note: CSV import (`import_csv`), `constraints` + `chase`, `coeval`, `schema_colimit`, and mappings with `lambda` expressions (used in the delta migration for flat reports) still require the `cql"""..."""` or `run_program` syntax. The DSL covers typeside, schema, and instance definitions. Queries for filtering cases (severe, hospital, flu, care home) can be expressed using the `@query` macro.
