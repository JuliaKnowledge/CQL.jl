...
also provides Julia-native macros. Here is how the Ecosystem schema and
...
The DSL also supports constraints, schema colimits, and functional
operators used in this vignette:

``` julia
# Constraints enforce ecological rules (e.g., every species needs a habitat):
# HabitatRequired = @constraints BioInventory """
#     forall s:Species -> exists hr:HabitatRecord where hr.species = s
# """

# Schema colimit merges monitoring programme schemas:
# Biodiversity = @schema_colimit Ty begin
#     @schemas MarineSurvey, ConservationDB
#     # Entity equations would identify shared concepts here
# end

# Functional API for data migration and merging:
#   Σ(AlignUni)(UniData)           — sigma pushforward
#   coproduct(StdUni, StdGov)      — merge aligned datasets
#   distinct(Combined)              — merge provably equal elements
```
...
definitions. Queries defined via `@query` can express the site and
method filters shown above.
