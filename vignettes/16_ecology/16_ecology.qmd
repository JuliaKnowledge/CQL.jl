...
The examples above use `run_program` with CQL source strings. CQL.jl also provides Julia-native macros. Here is how the Ecosystem schema looks using the DSL:
...
Note: CSV import (`import_csv`), `constraints` + `chase`, `coeval`, `schema_colimit`, and mappings with `lambda` expressions (used in the delta migration) still require the `cql"""..."""` or `run_program` syntax. The DSL covers typeside, schema, and instance definitions. Queries defined via `@query` can express the site and method filters shown above.
