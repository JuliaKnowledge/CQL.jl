...
get(Pkg.project().dependencies, "DuckDB", nothing) === nothing && Pkg.add("DuckDB")
using CQL, DuckDB
```
...
The examples above use the `cql"""..."""` string macro for schema and query definitions. CQL.jl also provides Julia-native macros. Here is how the Employee-Department schema looks using the DSL:
...
println("Foreign keys: ", collect(keys(S.fks)))
```

Note: DuckDB connectivity uses Julia functions (`import_duckdb`, `export_duckdb`) that work with schema objects directly, so the data I/O is already native Julia. The `@schema` macro defines the schema, and the programmatic import/export pattern shown above works the same way with DSL-defined schemas.
